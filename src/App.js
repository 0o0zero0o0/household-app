...
import { PieChart, Calendar, X, AlertCircle, Wallet, Utensils, List } from "lucide-react";
import "./App.css";
...
export default function App() {
  ...
  async function addTx(record) {
    await addDoc(collection(db, "tx"), { ...record, createdAt: Timestamp.now() });
    await loadTx();
  }

  useEffect(() => {
    // 로그인 여부와 무관하게 초기에 한 번 로드 (원하면 isAuthenticated 후로 바꿔도 됨)
  ...
  // ---- Static Budgets (샘플) ----
// ✅ Firestore에서 실제 데이터 합산 (없으면 0원으로 표시)
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-8">
          <div className="flex justify-center mb-6">
            <svg className="h-12 w-auto" viewBox="0 0 566.9 234.9" xmlns="http://www.w3.org/2000/svg">
              <path d="M565.1,234.9H1.8v-18.8h563.2V234.9z M348.8,167.3h-42.1v26.4h-16v-26.4h-42.1v-13h100.3V167.3z M555.6,193.6h-15.3V93
      ...
      <div className="w-full max-w-md flex flex-col items-center gap-6">
        ...
        <div className="w-full flex items-center justify-between">
          ...
          <div className="flex items-center justify-center">
            <svg className="h-8 md:h-10 w-auto" viewBox="0 0 566.9 234.9" xmlns="http://www.w3.org/2000/svg">
              <path d="M565.1,234.9H1.8v-18.8h563.2V234.9z M348.8,167.3h-42.1v26.4h-16v-26.4h-42.1v-13h100.3V167.3z M555.6,193.6h-15.3V93
                h15.3V193.6z M26.7,180.6H97v12.6H11.4v-32.7h15.3V180.6z M137.2,158.5h52.7v-9.8h15.3v44.5h-83.4v-44.5h15.3V158.5z M413.7,176.9
                h42.1v13H355.6v-13h42.1V155h16V176.9z M481.9,127.8h29.5V95.6h15.3v90.9h-60.2V95.6h15.3V127.8z M137.2,181h52.7v-10.4h-52.7V181z
                M481.9,173.7h29.5v-33h-29.5V173.7z M94.8,120.5h13.5v13.7H94.8v32.4H79.5V93h15.3V120.5z M413.6,108.5c0,4.9,1,9.4,2.9,13.5
...
function UserCard({ name, living, livingLimit, dining, diningLimit }) {
  const leftLiving = livingLimit - living;
  const leftDining = diningLimit - dining;
  ...
  return (
    <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-4">
      ...
      <div className="mb-4">
        ...
        <p className="text-xs text-green-600">남은 금액: {leftLiving.toLocaleString()}원</p>
      </div>
      <div>
        ...
        <p className="text-xs text-green-600">남은 금액: {leftDining.toLocaleString()}원</p>
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4 z-50">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          <button aria-label="닫기" onClick={onClose} className="w-8 h-8 grid place-items-center rounded-md hover:bg-gray-100">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <button className="px-2 py-1 rounded hover:bg-gray-50" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))}>◀</button>
            <div className="font-semibold">{year}년 {month + 1}월</div>
            <button className="px-2 py-1 rounded hover:bg-gray-50" onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))}>▶</button>
          </div>
            ...
            {grid.map((cell, idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-10 rounded-md text-sm relative border transition ${isSelected ? "border-blue-600" : "border-transparent"} hover:bg-gray-50`}
                >
                  <div className="flex items-center justify-center h-full">
                    <span className={`relative px-2 ${isToday ? "text-white" : ""}`}>
                      {cell.getDate()}
                      {isToday && <span className="absolute inset-0 -z-10 rounded-full bg-blue-600" />}
                    </span>
                  </div>
                  {sum > 0 && <span className="absolute bottom-1 left-1/2 -translate-x-1/2 h-1.5 w-1.5 rounded-full bg-green-600" />}
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-4 border-t pt-3 text-sm">
            <div className="font-medium mb-1">{selected.replaceAll("-", ".")} 사용 내역</div>
        ...
        <div className="p-4 space-y-4 text-sm">
          <div>
            ...
            <div className="flex items-center gap-3">
              <label className="inline-flex items-center gap-1"><input type="radio" checked={who === "all"} onChange={() => setWho("all")} /> 전체</label>
              <label className="inline-flex items-center gap-1"><input type="radio" checked={who === "지영"} onChange={() => setWho("지영")} /> 지영</label>
              <label className="inline-flex items-center gap-1"><input type="radio" checked={who === "지원"} onChange={() => setWho("지원")} /> 지원</label>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <label className="inline-flex items-center gap-1"><input type="radio" checked={period === "week"} onChange={() => applyPreset("week")} /> 이번 주</label>
              <label className="inline-flex items-center gap-1"><input type="radio" checked={period === "month"} onChange={() => applyPreset("month")} /> 이번 달</label>
              <label className="inline-flex items-center gap-1"><input type="radio" checked={period === "custom"} onChange={() => applyPreset("custom")} /> 직접 설정</label>
            </div>
            {period === "custom" && (
              <div className="mt-2 grid grid-cols-2 gap-2">
                <input type="date" value={start} onChange={(e) => setStart(e.target.value)} className="border rounded-md px-2 py-2" />
                <input type="date" value={end} onChange={(e) => setEnd(e.target.value)} className="border rounded-md px-2 py-2" />
              </div>
            )}
          </div>

          <div className="border rounded-lg divide-y max-h-72 overflow-auto">
            {rows.length === 0 && <div className="p-3 text-gray-500">해당 기간 내 기록 없음</div>}
            {rows.map((r, i) => (
              <div key={i} className="p-3 grid grid-cols-[88px_1fr_auto] items-center gap-2">
                <div className="text-gray-500 text-xs">{r.date.replaceAll("-", ".")}</div>
                <div className="text-xs">
                  <span className="text-gray-600 mr-1">{r.user}</span>
                  <span className="text-gray-800">{r.place} — {r.item}</span>
                </div>
                <div className="font-medium">₩{r.amount.toLocaleString("ko-KR")}</div>
              </div>
            ))}
          </div>
...
function EomSummaryModal({ onClose, tx }) {
  ...
  const totals = monthTx.reduce(
    (acc, t) => {
      acc.total += t.amount;
      acc.byUser[t.user] = (acc.byUser[t.user] || 0) + t.amount;
      acc.byCategory[t.category] = (acc.byCategory[t.category] || 0) + t.amount;
      acc.byPlace[t.place] = (acc.byPlace[t.place] || 0) + t.amount;
      return acc;
    },
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center p-4 z-50">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-4 text-sm space-y-3">
          <div className="flex items-center justify-between font-semibold"><span>이번 달 총 사용</span><span>₩{totals.total.toLocaleString("ko-KR")}</span></div>
          <div className="grid grid-cols-2 gap-2">
            <div className="border rounded-lg p-3">
              <div className="font-medium mb-2">사용자별</div>
              <div className="flex items-center justify-between"><span className="text-gray-600">지영</span><span>₩{totals.byUser.지영.toLocaleString("ko-KR")}</span></div>
              <div className="flex items-center justify-between"><span className="text-gray-600">지원</span><span>₩{totals.byUser.지원.toLocaleString("ko-KR")}</span></div>
            </div>
            <div className="border rounded-lg p-3">
              <div className="font-medium mb-2">카테고리별</div>
              <div className="flex items-center justify-between"><span className="text-gray-600">생활</span><span>₩{(totals.byCategory.생활 || 0).toLocaleString("ko-KR")}</span></div>
              <div className="flex items-center justify-between"><span className="text-gray-600">외식</span><span>₩{(totals.byCategory.외식 || 0).toLocaleString("ko-KR")}</span></div>
            </div>
          </div>
          <div className="border rounded-lg p-3">
            ...
            {topPlaces.map(([place, amt]) => (
              <div key={place} className="flex items-center justify-between"><span className="text-gray-600">{place}</span><span>₩{amt.toLocaleString("ko-KR")}</span></div>
            ))}
          </div>
        </div>
      </div>
...
function DateBreakdown({ selected, tx }) {
  const filtered = tx.filter((t) => t.date === selected);
  ...
  return (
    <div>
      <div className="flex items-center justify-between"><span className="text-gray-600">지영</span><span>₩{totalBy.byUser.지영.toLocaleString("ko-KR")}</span></div>
      <div className="flex items-center justify-between"><span className="text-gray-600">지원</span><span>₩{totalBy.byUser.지원.toLocaleString("ko-KR")}</span></div>
      <div className="flex items-center justify-between font-semibold mt-1"><span>합계</span><span>₩{totalBy.total.toLocaleString("ko-KR")}</span></div>
    </div>
  );
}
...
function summarizeByDate(tx) {
  ...
  for (const t of tx) {
    ...
    if (t.user === "지영" || t.user === "지원") map[key].byUser[t.user] += t.amount;
  }
  return map;
}
...
(function devTests() {
  try {
    console.assert(formatDate(new Date(2025, 0, 5)) === "2025-01-05", "formatDate 실패");
